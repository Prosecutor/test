        ...
        public void BuyShop(BuyPacket buyPacket)
        {
            ...
            switch (buyPacket.Type)
            {
                case BuyShopType.CharacterShop:
                    ...
                    if (sess == null || item == null || amount <= 0 || amount > 99)
                    {
                        return;
                    }
                ...
                case BuyShopType.ItemShop:
                    ...
                    MapNpc npc =
                        Session.CurrentMapInstance.Npcs.Find(n => n.MapNpcId.Equals((short) buyPacket.OwnerId));
                    if (npc != null)
                    {
        ...
        public void PetMove(PtCtlPacket ptCtlPacket)
        {
            ...
            for (int i = 0; i < ptCtlPacket.Amount * 3; i += 3)
            {
                                                                     ...
                                                                     out short positionY))
                {
                    ...
                    if (mate != null && Session.CurrentMapInstance?.Map?.IsBlockedZone(positionX, positionY) == false)
                    {
